...
- Bound each Python tool bridge bearer capability to one active session registration, preventing a capability exposed to one retained kernel from selecting another session through the request body; cleanup invalidates stale capabilities and kernel rotation issues fresh authority.
